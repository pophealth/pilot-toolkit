module Stats
  ...
  # PatientSummaryReport is used to capture an assesment of coded values within both 
  # the HITSP C32 and ASTM CCR documents that are to be input to popHealth
  class PatientSummaryReport
    ...
    # Note that we add an accessor method when the section is added...this greatly 
    # simplifies methods that iterate over all existing sections
   ...
   def self.from_c32(document)
    ...
    # from_ccr:  read a Nokogiri::Document, and leverage the CCRScan::CCR 
    # to build a hash just like the one created by QME patient importer, 
    # a hash of entries broken down by section.  Pass these through the 
    # PatientSummarySection analysis
    # @param [Nokogiri::Document]   source document
